                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-12 col-lg-3">
                            <img class="speaker-img-card img-fluid" role="presentation" alt="" src="${speaker.imageUrl}">
                        </div>
                        <div class="col-12 col-lg-9">
                            <h3 class="pt-3 pt-lg-0 mb-0"><a class="blue" href="speaker-details.html?id=${speaker.id}">${speaker.name}</a></h3>
...
function generateSpeakerDetails(speaker) {
    ...
    var sessionTitleHTML = speaker.liveSessionURL ?
        `<h4 class="mb-0"><a href="${speaker.liveSessionURL}" target="_blank" class="blue">${speaker.session}</a></h4>` :
        `<h4 class="mb-0">${speaker.session}</h4>`;
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    filteredSpeakers.forEach(speaker => {
        ...
        sessions[speaker.session].push(speaker);
    });

    // Function to generate HTML for each agenda item
    function generateAgendaItem(session, speakers) {
        var speakersList = speakers.map(speaker => `<a href="speaker-details.html?id=${speaker.id}">${speaker.name}</a>`).join(', ');
        return `
    ...
    // Iterate through each session and its associated speakers to generate agenda items
